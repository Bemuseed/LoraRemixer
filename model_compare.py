...
PATHS=["vincent_di_fate.ckpt", "david_mack.ckpt", "alan_lee.ckpt", "ed_mell.ckpt", "michael_garmash.ckpt"]
...
def process(model_paths):
    ...
    for i in range(len(model_combos)):
        ...
        name_a, name_b = remove_filetype(model_paths[ind_a]), remove_filetype(model_paths[ind_b])
        merged_model_names.append("(" + name_a + ")" + "-" + "(" + name_b + ")" + "_0.5")
    ...
    sorted_models_and_scores = sorted(zip(combo_difference_scores, merged_models, merged_model_names), reverse=True)
    for i in sorted_models_and_scores:
        print(i[2])
    ...
    for i in tqdm(range(3), "Saving three most different models to disk"):
        torch.save(merged_models[i], open(merged_model_names[i], "wb"))
